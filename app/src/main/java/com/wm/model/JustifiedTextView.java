...
public class JustifiedTextView extends View {
    ...
    Typeface font;
    int textColor;
    float textSize = 42f, lineHeight = 57f, wordSpacing = 15f,
	    lineSpacing = 15f;
    float onBirim, w, h;
	...
	for (String line : lines) {
		...
		if (lineWidth + ww
			+ (lineBuffer.getWords().size() * wordSpacing) > w) {// is
		    lineBuffer.addWord(word);
		    totalWordWidth += textPaint.measureText(word);
		    lineBuffer
			    .setSpacing((w - totalWordWidth - leftPadding - rightPadding)
				    / (lineBuffer.getWords().size() - 1));
	...
	setMeasuredDimension((int) w, (int) ((this.linesCollection.size() + 1)
		* lineHeight + (10 * onBirim)));
    }
